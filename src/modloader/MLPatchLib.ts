...
import { RomPatchType } from 'API/build/Patchers/PatchManager';
...
export class MLPatchLib {
    create(rom_name: string, og: Buffer, mod: Buffer): Buffer {
        ...
        let per: number = -1;
        let byteThrottle = 6;
        let throttleLimit = byteThrottle;
        let lastFind = -1;
...
export class MLPatchLibReg implements RomPatchType{

    patch(rom: Buffer, patch: Buffer): Buffer {
        let patcher: MLPatchLib = new MLPatchLib();
        return patcher.apply(rom, patch);
    }
