...
import { CoreEvent, CoreParam, IRetro } from "./IRetro";
        ...
        try {
            ...
            this.Binding.on('core-event', (event: CoreEvent, v: number) => {
                if (event == CoreEvent.VolumeDown)
                    (this.Binding as any).retro.Retro.setAudioVolume((this.Binding as any).retro.Retro.getAudioVolume() - volumeAdjust);
        ...
        if (this.ModLoader.ImGui.beginMainMenuBar()) {
            if (this.ModLoader.ImGui.beginMenu("Emulation")) {
                if (this.ModLoader.ImGui.combo('Aspect ratio', this.aspect, this.aspect_options)) {
                    //((this.Binding as any)["retro"] as IRetro).Retro.Config.openSection('Video-GLideRetro').setInt('AspectRatio', this.aspect[0]);
                }
                if (this.ModLoader.ImGui.inputInt("Screen Width", this.ScreenWidth)) {
                    let w = Math.floor(this.ScreenWidth[0]);
                    //((this.Binding as any)["retro"] as IRetro).Retro.Config.openSection("Video-General").setInt("ScreenWidth", w);
                }
                if (this.ModLoader.ImGui.inputInt("Screen Height", this.ScreenHeight)) {
                    let w = Math.floor(this.ScreenHeight[0]);
                    //((this.Binding as any)["retro"] as IRetro).Retro.Config.openSection("Video-General").setInt("ScreenHeight", w);
                }
                if (this.ModLoader.ImGui.beginMenu("Enable High Res Texture Packs")) {
                    if (this.ModLoader.ImGui.menuItem("Rice/HTC", undefined, this.htc, true)) {
                        this.htc = !this.htc;
                        //((this.Binding as any)["retro"] as IRetro).Retro.Config.openSection('Video-GLideRetro').setBool('txHiresEnable', true);
                        //((this.Binding as any)["retro"] as IRetro).Retro.Config.openSection('Video-GLideRetro').setInt('txCacheSize', 1000);
                        //((this.Binding as any)["retro"] as IRetro).Retro.Config.openSection('Video-GLideRetro').setBool('txHiresFullAlphaChannel', true);
                        //((this.Binding as any)["retro"] as IRetro).Retro.Config.openSection('Video-GLideRetro').setBool('txEnhancedTextureFileStorage', false);
                        //((this.Binding as any)["retro"] as IRetro).Retro.Config.openSection('Video-GLideRetro').setBool('txHiresTextureFileStorage', false);
                        //((this.Binding as any)["retro"] as IRetro).Retro.Config.openSection('Video-GLideRetro').setBool('txSaveCache', true);
                    }
                    if (this.ModLoader.ImGui.menuItem("HTS", undefined, this.hts, true)) {
                        ...
                        //((this.Binding as any)["retro"] as IRetro).Retro.Config.openSection('Video-GLideRetro').setInt('txCacheSize', 1000);
                        //((this.Binding as any)["retro"] as IRetro).Retro.Config.openSection('Video-GLideRetro').setBool('txHiresFullAlphaChannel', true);
                        //((this.Binding as any)["retro"] as IRetro).Retro.Config.openSection('Video-GLideRetro').setBool('txEnhancedTextureFileStorage', true);
                        //((this.Binding as any)["retro"] as IRetro).Retro.Config.openSection('Video-GLideRetro').setBool('txHiresTextureFileStorage', true);
                    }
                    this.ModLoader.ImGui.endMenu();
                }
                if (this.ModLoader.ImGui.menuItem("FXAA", undefined, this.fxaa, true)) {
                    this.fxaa = !this.fxaa;
                    //((this.Binding as any)["retro"] as IRetro).Retro.Config.openSection('Video-GLideRetro').setBool('FXAA', this.fxaa);
                }
                if (this.ModLoader.ImGui.combo("Max Anisotropy", this.ani, this.ani_options)) {
                    if (this.ani[0] === 0) {
                        //((this.Binding as any)["retro"] as IRetro).Retro.Config.openSection('Video-GLideRetro').setInt('MaxAnisotropy', 0);
                    }
                    if (this.ani[0] === 1) {
                        //((this.Binding as any)["retro"] as IRetro).Retro.Config.openSection('Video-GLideRetro').setInt('MaxAnisotropy', 2);
                    }
                    if (this.ani[0] === 2) {
                        //((this.Binding as any)["retro"] as IRetro).Retro.Config.openSection('Video-GLideRetro').setInt('MaxAnisotropy', 4);
                    }
                    if (this.ani[0] === 3) {
                        //((this.Binding as any)["retro"] as IRetro).Retro.Config.openSection('Video-GLideRetro').setInt('MaxAnisotropy', 8);
                    }
                    if (this.ani[0] === 4) {
                        //((this.Binding as any)["retro"] as IRetro).Retro.Config.openSection('Video-GLideRetro').setInt('MaxAnisotropy', 16);
                    }
                }
                if (this.ModLoader.ImGui.combo("MultiSampling", this.ms, this.ms_options)) {
                    if (this.ms[0] === 0) {
                        //((this.Binding as any)["retro"] as IRetro).Retro.Config.openSection('Video-GLideRetro').setInt('MultiSampling', 0);
                    }
                    if (this.ms[0] === 1) {
                        //((this.Binding as any)["retro"] as IRetro).Retro.Config.openSection('Video-GLideRetro').setInt('MultiSampling', 2);
                    }
                    if (this.ms[0] === 2) {
                        //((this.Binding as any)["retro"] as IRetro).Retro.Config.openSection('Video-GLideRetro').setInt('MultiSampling', 4);
                    }
                    if (this.ms[0] === 3) {
                        //((this.Binding as any)["retro"] as IRetro).Retro.Config.openSection('Video-GLideRetro').setInt('MultiSampling', 8);
                    }
                    if (this.ms[0] === 4) {
                        //((this.Binding as any)["retro"] as IRetro).Retro.Config.openSection('Video-GLideRetro').setInt('MultiSampling', 16);
                    }
                }
                if (this.ModLoader.ImGui.combo("Audio", this.audio_selection, this.audio_options_display)) {
                    //((this.Binding as any)["retro"] as IRetro).Retro.Config.openSection("Audio-SDL").setString("RESAMPLE", this.audio_options[this.audio_selection[0]]);
                }
