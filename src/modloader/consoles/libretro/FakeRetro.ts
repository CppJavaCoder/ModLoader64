...
import { FakeRetroMemory } from './FakeRetroMemory';
...
export class FakeRetro implements IConsole {
    rom: string;
    rom_data: Buffer;
    ram: FakeRetroMemory;
    ...
    getRomHeader(): IRomHeader {
        return new RetroHeader(Buffer.alloc(0x50));
    }
    ...
    getUtils(): IUtils {
        return new FakeRetroUtils();
    }

    getSaveStateManager(): ISaveState {
        return new FakeRetroSaveState();
    }
}

class FakeRetroSaveState implements ISaveState {
...
class FakeRetroUtils implements IUtils {
